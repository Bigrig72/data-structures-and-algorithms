...
const forEachTwoToThe = (arr) => {
  console.log(arr);
  let newArr = [];
  arr.forEach(exponent => {
    newArr.push(Math.pow(2, exponent));
  })
  return newArr;
}
...
Write a function named mapTwoToThe that produces the same 
output as your forLoopTwoToThe function from challenge 1 and
 your forEachTwoToThe function from challenge 2, but uses map 
 instead of a for loop or forEach.
------------------------------------------------------------------------------------------------ */

const mapTwoToThe = (arr) => {
 let num = arr.map(n => Math.pow(2, n));
 return num;
...
uses map to return a new array where each element is the result of the `charCodeAt`
...
const charCode = (arr) => {   
 let letter = arr.map(n => arr.charCodeAt(n));
 return letter;
};
